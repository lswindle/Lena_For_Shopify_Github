...
class CartCount extends HTMLElement {
  ...
  attachEvents() {
    document.addEventListener("ARMADA:CART:COUNT:CHANGE", (e) => this.handleCartCountChange(e.detail));
    window.eight.armada.elementRegistry.register({key: 'component-cart-count', assetPath: '/assets/component-cart-count.min.js'});
  }
  ...
  handleCartCountChange({ count }) {
    this.setCount(Number(count));
  }

  setCount(count) {
    this.countElem.textContent = count;
    count === 0 ? this.classList.add('hidden') : this.classList.remove('hidden');
  }
