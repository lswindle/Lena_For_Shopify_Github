...
class LocalizationForm extends HTMLElement {
  constructor () {
    super();
    this.input = this.querySelector('input[name="country_code"], input[name="locale_code"]');
  ...
  init() {
    window.eight.armada.elementRegistry.register({key: 'localization-form', assetPath: '/assets/component-armament-localization-form.min.js'});
    this.button.addEventListener('click', this.showOptions.bind(this));
    this.button.addEventListener('focusout', this.closeOptions.bind(this));
  ...
  showOptions() {
    this.button.focus();
    this.querySelector('ul').toggleAttribute('hidden');
    this.button.setAttribute('aria-expanded', (this.button.getAttribute('aria-expanded') === 'false').toString())
    window.eight.eventsEngine.emit(`${eight.constants.events["ARMADA:NAVIGATION:DROPDOWN:CLOSE"]}`)
  }

  hideOptions() {
    this.button.setAttribute('aria-expanded', 'false');
    this.querySelector('ul').setAttribute('hidden', true);
  }
