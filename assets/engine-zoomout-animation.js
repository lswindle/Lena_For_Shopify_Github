...
2) Make sure to set zoomout_animation to `true` when you render 'armament-image'

3) If you need to fade-up the text that goes along with the img, add `{% if settings.enable_animations %}data-fadeup-animation{% endif %}` to the text wrapper.
 */

class ZoomoutAnimation extends HTMLElement {
  constructor() {
    super();
  ...
  init() {
    window.eight.armada.elementRegistry.register({key: 'zoomout-animation', assetPath: '/assets/engine-zoomout-animation.min.js'});
  }

  observerCallback(entry, observer) {
    if (entry[0].isIntersecting) {
      this.setAttribute('data-zoomout-animated', `${entry[0].isIntersecting}`);
      this.observer.unobserve(this)
    }
  }
}

window.customElements.define('zoomout-animation', ZoomoutAnimation);
