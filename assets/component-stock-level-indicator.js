...
class StockLevelIndicator extends HTMLElement {
  ...
  init() {
    window.eight.armada.elementRegistry.register({key: 'stock-level-indicator', assetPath: '/assets/component-stock-level-indicator.min.js'});
  }
  ...
  attributeChangedCallback(attribute, oldVal, newVal) {
    ...
    const oldVariant = this.querySelector(`[data-armada-selector="stock-level-indicator--${oldVal}"]`);
    if (oldVariant) {
      oldVariant.classList.add('hidden');
    }

    const newVariant = this.querySelector(`[data-armada-selector="stock-level-indicator--${newVal}"]`);
    if (newVariant) {
...
window.customElements.define('stock-level-indicator', StockLevelIndicator);
