...
class FramedImg extends HTMLElement {
    constructor() {
        ...
        this.observer = new IntersectionObserver((entries, observer) => this.observerCallback(entries, observer), this.observerOptions);
    ...
    static get observedAttributes() { return [...Object.keys(changeSet)]; }
    ...
    init() {
        const overlayContentElement = document.querySelector(`[data-armada-selector="mobile-overlay-content-${this.targetSelector}" ]`);
        const desktopContentElement = document.querySelector(`[data-armada-selector="desktop-overlay-${this.targetSelector}"]`);
        desktopContentElement.innerHTML = overlayContentElement.innerHTML;
        window.eight.armada.elementRegistry.register({ key: 'framed-image', assetPath: '/assets/component-framed-image.min.js' });
    }
    observerCallback(entries, observer) {
        ...
        if (this.frameType === 'full-width') {
            this.setAttribute('data-framed-out', lessThanThreeQuarters ? 'true' : 'false');
    ...
    changeBorder(change) {
        const contentContainer = this.querySelector('[data-armada-selector="framed-content-container"]');
        const mobileContainer = this.querySelector(`[data-armada-selector="mobile-overlay-content-${this.targetSelector}"]`);
        if (change) {
            contentContainer.classList.add(...this.activeFrameClasses.split(' '));
            contentContainer.classList.remove(...this.defaultFrameClasses.split(' '));
            mobileContainer.classList.add(...this.mobileActiveFrameClasses.split(' '));
            mobileContainer.classList.remove(...this.mobileDefaultFrameClasses.split(' '));
        }
        else {
            contentContainer.classList.remove(...this.activeFrameClasses.split(' '));
            contentContainer.classList.add(...this.defaultFrameClasses.split(' '));
            mobileContainer.classList.remove(...this.mobileActiveFrameClasses.split(' '));
            mobileContainer.classList.add(...this.mobileDefaultFrameClasses.split(' '));
        }
        const allBlocks = document.querySelectorAll('[data-only-display-active="true"]');
        if (allBlocks.length > 0) {
            allBlocks.forEach(element => {
                element.setAttribute('data-display-status', change ? 'true' : 'false');
            });
        }
    }
    isMobile() {
