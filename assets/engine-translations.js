...
class TranslationEngine extends HTMLElement {
  constructor() {
    super();

    this.templateContent = this.querySelector('[data-armada-selector="translations-engine-template"]').content;
  }
  ...
  init() {
    // Read the theme's locale strings (from engine-translations.liquid) and store on window.eight.translationEngine.strings
    window.eight.translationEngine.storeLocaleJson(this.jsonObject);
  }

  get jsonObject() {
    const scriptJson = this.templateContent.querySelector(`[data-armada-selector="translations-json"]`).innerHTML;
    return window.eight.safeJSONParse(scriptJson);
  }
}

customElements.define('translation-engine', TranslationEngine);
