...
class complementaryProducts extends HTMLElement {
  ...
  init() {
    window.eight.armada.elementRegistry.register({key: 'complementary-products', assetPath: '/assets/component-complementary-products.min.js'});
    fetch(this.dataset.url)
      ...
      .then((text) => {
        ...
        if (recommendations && recommendations.innerHTML.trim().length) {
          ...
          if (this.parentEl) this.parentEl.classList.remove('invisible', 'h-0');
          if (this.parentEl) this.parentEl.classList.add('lg:mb-xl', 'lg:mx-0', 'lg:mt-0');
        }
      })
      .catch((e) => {
        console.error(e);
