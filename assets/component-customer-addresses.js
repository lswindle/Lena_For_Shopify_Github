...
class deleteAddress extends HTMLElement {
    ...
    connectedCallback() {
        this.deleteAddressForm = this.querySelector('form');
        (0,_weareeight_armada_dist_utils_isArmadaLoaded__WEBPACK_IMPORTED_MODULE_0__.isArmadaLoaded)(this.init.bind(this));
    }

    init() {
        this.deleteAddressForm.addEventListener('submit', this.handleDeleteBtn);
    ...
    handleDeleteBtn(event) {
        if (confirm(this.getAttribute('data-confirm-message'))) {
            ...
            this.addressCard = this.closest('li');
            this.buttonText = this.querySelector('button [data-armada-button-text]');
            this.buttonLoadIcon = this.querySelector('button [data-armada-button-load-icon]');

            this.buttonText.classList.add('invisible');
            this.buttonLoadIcon.classList.remove('hidden');
            this.addressCard.classList.add('opacity-40');

            //Set the url equal to the form action
            ...
            .then(() => {
                window.eight.sectionsEngine.sectionRenderingEngine.fetchSection(window.location.pathname, ['customer--addresses'], [document.querySelector('customer-addresses')], ['customer-addresses']);
            })
